 ...
 *
 * Zero tolerance — no allowlist. The two `strncpy` mentions that remain in the
 * tree are gcc -fanalyzer workaround comments explaining a deliberate non-use;
 * the scanner only matches call syntax `strncpy(`, so prose is out of scope by
 * construction.
 *
...
const CALL_RE = /\bstrncpy\s*\(/;

function listCSources(dir) {
  return fs
    .readdirSync(dir, {withFileTypes: true})
    .filter(e => e.isFile() && /\.c$/.test(e.name))
    .map(e => path.join(dir, e.name))
    .sort();
}

/**
 * Find every line carrying a `strncpy(` call in the given C sources. Non-.c
 * paths are ignored so callers cannot widen the scope by accident.
 */
