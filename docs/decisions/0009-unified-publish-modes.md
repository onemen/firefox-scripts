...
One pipeline, `node tools/publish/upload.mjs`, run as `publish:*` / `snapshot:*` with a **required**
...
served via jsDelivr. `snapshot:*` writes a full offline snapshot to `dist/<mode>-<branch>-<hash>/`
and never touches the token. Token validation applies to real uploads only: a missing/empty token
fails the publish fast before building anything (`46ad7c3`, 2026-08-09; consolidated `21a3304` /
`ca54de6` / `51a5d1e`).
