...
// Shared with restart.c (declared in restart.h).
char g_ui_url[128];
...
char g_ui_host_profile[MAX_PATH_LEN] = "";
...
RunningBrowser detected_browsers[MAX_BROWSERS];
...
int session_installed_config[MAX_BROWSERS];
int session_installed_utils[MAX_BROWSERS];
...
int handle_api_restart(int client_fd, const char *query, const char *body, size_t body_len) {
    ...
    int async = 0;
#ifdef _WIN32
    async = restart_start_async(&plan);
#endif
    ...
    } else if (ui_host_killed && restart_worker_busy()) {
